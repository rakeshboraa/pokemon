...
export default function PokemonCard({name, url,index = 0}: PokemonCardProps) {
  ...
  return (
    <Link href={`/pokemon/${name}`}
      className="group pokemon-card w-full overflow-hidden rounded-xl p-3 transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_8px_0_#d4a900] sm:rounded-2xl sm:p-4 sm:hover:-translate-y-2 sm:hover:shadow-[0_12px_0_#d4a900]"
      style={{
        perspective: "1000px",
        animationDelay: `${Math.min(index * 40, 400)}ms`,
      }}
    >
      <div className="transition-transform duration-500 ease-out group-hover:transform-[rotateX(5deg)_rotateY(-5deg)]" style={{ transformStyle: "preserve-3d" }}>
